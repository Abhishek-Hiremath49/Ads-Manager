...
class AdsSetting(Document):
    ...
    def can_launch_campaign(self, platform: str) -> bool:
        if platform == "Facebook":
            return (self.facebook_launches_today or 0) < self.facebook_daily_launch_limit
        elif platform == "Instagram":
            return (self.instagram_launches_today or 0) < self.instagram_daily_launch_limit
        return True
    
    def increment_launches(self, platform: str):
        if platform == "Facebook":
            self.facebook_launches_today = (self.facebook_launches_today or 0) + 1
        elif platform == "Instagram":
            self.instagram_launches_today = (self.instagram_launches_today or 0) + 1
        self.save(ignore_permissions=True)
    
    def reset_daily_counters(self):
        """Reset daily counters - called by scheduled job"""
        self.facebook_launches_today = 0
        self.instagram_launches_today = 0
